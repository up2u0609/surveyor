...
describe Surveyor::Dependency, "when evaluating dependency conditions of a question in a response set" do
  ...
  before(:each) do
    @dep = Surveyor::Dependency.make(:rule => "A", :question_id => 1)
    @dep2 = Surveyor::Dependency.make(:rule => "A and B", :question_id => 1)
    @dep3 = Surveyor::Dependency.make(:rule => "A or B", :question_id => 1)
    @dep4 = Surveyor::Dependency.make(:rule => "!(A and B) and C", :question_id => 1)
...
describe Surveyor::Dependency, "with conditions" do
  it "should destroy conditions when destroyed" do
    @dependency = Surveyor::Dependency.make(:rule => "A and B and C", :question_id => 1)
    Surveyor::DependencyCondition.make(:dependency => @dependency, :rule_key => "A")
    Surveyor::DependencyCondition.make(:dependency => @dependency, :rule_key => "B")
